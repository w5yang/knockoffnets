...
import neotheft.models.custom_cnn


def get_net(modelname: str, modeltype: str, pretrained: str = None, **kwargs) -> nn.Module:
    assert modeltype in ('custom_cnn')
    if pretrained and pretrained is not None:
        ...
        try:
            ...
            model = eval('neotheft.models.{}.{}'.format(modeltype, modelname))(**kwargs)
            ...
            model = eval('neotheft.models.{}.{}'.format(modeltype, modelname))()
...
def get_pretrainednet(modelname, modeltype, pretrained='imagenet', num_classes=1000, **kwargs):
    if osp.exists(pretrained):
        model = eval('neotheft.models.{}.{}'.format(modeltype, modelname))(num_classes=num_classes, **kwargs)
